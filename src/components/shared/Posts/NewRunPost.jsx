...
import RunTrainingPanel from "../../run/RunHistory/RunTrainingPanel"

const NewRunPost = ({ toggleRun, postAdded }) => {
  ...
  const sendPost = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          gym_workout_id: null,
          run_workout_id: selectedWorkout,
  ...
  const fetchWorkouts = async (currentPage) => {
    ...
    try {
      ...
      console.error("Error fetching run workouts:", error);
    }
  };
  ...
  const handleWorkoutSelect = (workoutId) => {
    setSelectedWorkout((prevSelected) =>
      prevSelected === workoutId ? null : workoutId
    );
  };
        ...
        <div className="w-full flex flex-col items-center justify-center gap-2">
          ...
          <div className="flex justify-center flex-wrap">
            {workouts.map((workout) => (
              <div
                key={workout.run_workout_id}
                onClick={() => handleWorkoutSelect(workout.run_workout_id)}
                className={`${
                  selectedWorkout === workout.run_workout_id ? "bg-red-500" : ""
                }`}
              >
                <RunTrainingPanel
                  key={workout.run_workout_id}
